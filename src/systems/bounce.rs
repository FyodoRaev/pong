      ...
      for (ball, transform) in (&mut balls, &transforms).join() {
          ...
          for (paddle, paddle_transform) in (&paddles, &transforms).join() {
              ...
              if point_in_rect(
                  ...
                  if (paddle.side == Side::Left && ball.velocity_x < 0.0)
                      || (paddle.side == Side::Right && ball.velocity_x > 0.0)
                  {
                      ball.velocity_x = -ball.velocity_x;
                      ball.velocity_x = ball.velocity_x + ball.velocity_x.signum()*45.0;
                      ball.velocity_y = ball.velocity_y + ball.velocity_y.signum()*45.0;
                  }
              }
          }
       }
